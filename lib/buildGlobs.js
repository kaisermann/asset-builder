...
var buildGlobs = module.exports = function (types, assets, bowerFiles, options) {
	...
	_.forOwn(this.types, function (typeGlob, typeName) {
		_this.globs[typeName] = _this.getOutputFiles(typeName, typeGlob, assets, bowerFiles)
	}, this);
...
buildGlobs.prototype.getOutputFiles = function (typeName, typeGlob, assets, bowerFiles) {
	...
	outputFiles = _.pickBy(assets[typeName], function (asset, name) {
		return helpers.isDir(name) || minimatch(name, typeGlob, { matchBase: true });
	});
	...
	outputFiles = _.transform(outputFiles, function (result, asset, name) {
		...
		var bower = [];
		var bowerExclude = _this.bowerExclude(assets[typeName]);
