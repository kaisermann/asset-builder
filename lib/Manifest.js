...
var Manifest = module.exports = function (path, options) {
	...
	this.globs = new BuildGlobs(manifest.config.fileTypes, manifest.dependencies, bower, options).globs;
	this.paths = manifest.config.paths;
};
...
Manifest.prototype.getDependencyByName = function (name) {
  return _.find([].concat(this.globs.scripts, this.globs.styles), {name: name});
};
...
Manifest.prototype.getProjectGlobs = function () {
	return _.reduce(this.dependencies, function (result, deps, depType) {
		_.forEach(deps, function (dep, key) {
			var type = Dependency.parseType(depType);
